...
static void U_CALLCONV unitPreferencesTestDataLineFn(void *context,
  ...
  if (status.errIfFailureAndReset("parsing decimal quantity: \"%.*s\"",
                                  outputD.length(), outputD.data())) {
    return;
  }
  double expectedOutput = dqOutputD.toDouble();

  MeasureUnit input = MeasureUnit::forIdentifier(inputUnit, status);
  if (status.errIfFailureAndReset("forIdentifier(\"%.*s\")", inputUnit.length(),
                                  inputUnit.data())) {
    return;
  }

  MeasureUnit output = MeasureUnit::forIdentifier(outputUnit, status);
  if (status.errIfFailureAndReset("forIdentifier(\"%.*s\")",
                                  outputUnit.length(), outputUnit.data())) {
    return;
  }
  ...
  fprintf(stderr,
          ...
          quantity.length(), quantity.data(), usage.length(), usage.data(),
          region.length(), region.data(),
          inputD.length(), inputD.data(), inputUnit.length(), inputUnit.data(),
          inputR.length(), inputR.data(),
          outputD.length(), outputD.data(), outputUnit.length(), outputUnit.data(),
          outputR.length(), outputR.data(), expectedOutput);
}
...
void UnitsTest::testPreferences() {
  ...
  // number of fields.
  u_parseDelimitedFile(path.data(), ';', fields, kNumFields,
                       unitPreferencesTestDataLineFn, this, errorCode);
  if (errorCode.errIfFailureAndReset("error parsing %s: %s\n", path.data(),
                                     u_errorName(errorCode))) {
    return;
  }
