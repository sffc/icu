...
void NumberRangeFormatterImpl::formatSingleValue(UFormattedNumberRangeData& data,
    ...
    if (U_FAILURE(status)) { return; }
    if (fSameFormatters) {
        int32_t length = NumberFormatterImpl::writeNumber(micros1.toSimple(), data.quantity1, data.getStringRef(), 0, status);
...
void NumberRangeFormatterImpl::formatApproximately (UFormattedNumberRangeData& data,
    ...
    if (fSameFormatters) {
        ...
        int32_t length = NumberFormatterImpl::writeNumber(microsAppx.toSimple(), data.quantity1, data.getStringRef(), 0, status);
...
void NumberRangeFormatterImpl::formatRange(UFormattedNumberRangeData& data,
    ...
    length1 += NumberFormatterImpl::writeNumber(micros1.toSimple(), data.quantity1, string, UPRV_INDEX_0, status);
    // ICU-21684: Write the second number to a temp string to avoid repeated insert operations
    FormattedStringBuilder tempString;
    NumberFormatterImpl::writeNumber(micros2.toSimple(), data.quantity2, tempString, 0, status);
