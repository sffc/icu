...
def generate_index_file(locales, common_vars):
    ...
    return ("// Warning this file is automatically generated\n"
            ...
            "    CLDRVersion {{ \"{GENRB_CLDR_VERSION}\" }}\n"
            "    InstalledLocales {{\n"
            "{FORMATTED_LOCALES}\n"
            "    }}\n"
            "}}").format(**common_vars, FORMATTED_LOCALES = formatted_locales)
...
def main():
    ...
    specialized_sub_dirs = [
        # (input dirname, output dirname, genrb options, dep files)
        ("locales",  None,       "--usePoolBundle", []),
        ("curr",     "curr",     "--usePoolBundle", []),
        ("lang",     "lang",     "--usePoolBundle", []),
        ("region",   "region",   "--usePoolBundle", []),
        ("zone",     "zone",     "--usePoolBundle", []),
        ("unit",     "unit",     "--usePoolBundle", []),
        ("coll",     "coll",     "",                ["coll/ucadata.icu"]),
        ("rbnf",     "rbnf",     "",                []),
        ("translit", "translit", "",                [])
    ]
    for sub_dir, out_sub_dir, extra_options, dep_files in specialized_sub_dirs:
        ...
        if config.has_feature(sub_dir):
            ...
            # This will be required for Bazel.
            mk_values = parse_makefile("{IN_DIR}/{IN_SUB_DIR}/resfiles.mk".format(**common, IN_SUB_DIR = sub_dir))
            ...
            requests += [
                ExecutionRequest(
                    ...
                    tool = "echo",
                    args = generate_index_file(locales, common)
                )
            ]
